...
public class Zoo {
    ...
    public List<Animal> recuperaAnimals() throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Animal animal = new Animal(bdId, nom, obteCategoriaPerId(catId));
                animals.add(animal);
            }
            rs.close();
            if (animals.isEmpty()) {
    ...
    public Animal obteAnimalPerNom(String p_nom) throws SQLException {
        String sql = String.format(
                "SELECT * from ANIMALS where '%s' = nom ORDER BY id LIMIT 1;", p_nom
                );
        Statement st = null;
        try {
            st = conn.createStatement();
            ResultSet rs = st.executeQuery(sql); 
            if (rs.getString("nom") == null || rs.getString("nom").isEmpty()) return null;
             
            int bdId = rs.getInt("id");
            String nom = rs.getString("nom");
            int catId = rs.getInt("categoria");
            rs.close();
            return new Animal(bdId, nom, obteCategoriaPerId(catId));
        } finally {
            if (st != null) st.close();
        }
    }
    
    public Categoria obteCategoriaPerId(int p_id) throws SQLException {
        String sql = String.format(
                "SELECT * FROM CATEGORIES WHERE id = '%d' ORDER BY id LIMIT 1;",
                p_id);
        Statement st = null;
        try {
            st = conn.createStatement();
